...
static void dbread(const string& filename, vector<Mat>& images, vector<int>& labels, char separator = ','){
	...
	if (!file){
		string error = "I am stuck here! no valid input file!";
...
void fisherFaceTrainer(){
	...
	try{		
		...
		string filename = "F:/Data Base/at.csv";
		dbread(filename, images, labels);
...
int  FaceRecognition(){
	...
	Ptr<FaceRecognizer>  model = createFisherFaceRecognizer();
	model->load("F:/Data Base/fisherface.yml");

	Mat testSample = imread("F:/Data Base/Shahzeb/00.png", 0);
